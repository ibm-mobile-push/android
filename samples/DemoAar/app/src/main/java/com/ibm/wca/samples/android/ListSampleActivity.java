...
import com.ibm.wca.samples.android.layout.CustomListAdapter;
...
public abstract class ListSampleActivity extends SampleActivity implements AdapterView.OnItemClickListener, CustomListAdapter.ValueChangeListener {
    ...
    protected void handleActionIntent(Intent intent) {
        if(intent != null && intent.getExtras() != null) {
            if(intent.hasExtra(WcaNotifier.ACTION_KEY)) {
                String action = intent.getStringExtra(WcaNotifier.ACTION_KEY);
                if (action != null) {
                    if (WcaNotifier.ACTION_SDK_REGISTRATION.equals(action)) {
                        handleSdkRegistration();
                    } else if (WcaNotifier.ACTION_GCM_REGISTRATION.equals(action)) {
